...
export default class AppBody extends React.Component<any,any> {
     ...
     */
    render() {
        let {children,meu_reducers,hed_reducers,actions,menuComponent} = this.props;
        let Cls = meu_reducers.menuSwitch ? adCls : adCls + " off";
        return (<div className={`${css_prefix}-body`}>
                    <AppHeader menuComponent={menuComponent} meu_reducers={meu_reducers} hed_reducers={hed_reducers} actions = {actions}/>
                    <div className = { Cls }>
                        <AppMenu />
